...
# engine — every scan is based on the current file state and needs no cross-session
# persistent state. memory: project would implicitly enable Write/Edit, contradicting
# disallowedTools.
maxTurns: 15
---
...
- Paragraphs break naturally on dramatic unit / shot / one thing ending, with no mechanical word-count splitting; paragraph spacing follows the book/platform contract with no missing or stray blank-line runs; dialogue on its own lines; natural subject/character-name rhythm

...
### Post-Write Style Self-Check (mandatory when a benchmark book or `style.md` exists — prevents style drift in continuations)
...
1. Take the target sentence-length band from the authoritative `setting/style.md` or benchmark `style.md`; never read a style fingerprint from `tracking/context.md`, which stores progress metadata only. If neither style source has numeric guidance, rely on the feel of the original anchor excerpts.
...
3. If hit, judge **style drift**: merge fragments into medium-long sentences along the target band, restore imagery and connectors, recover narrative breathing, rearrange, then deliver. **Continuation carries the plot forward, not the previous chapter's sentence rhythm; style follows `style.md` and original anchors.**

`tracking/context.md` is a progress card, not a style store. The continuation state card does not contain a style fingerprint.
...
- Format compliance: break paragraphs on action/information change, control per-paragraph density; adjacent prose paragraphs use exactly one blank line (`\n\n`) — no extra blank-line runs; dialogue on its own lines; avoid formulaic high-frequency dialogue tags, plain "said" may stay
...
- **Readable paragraph spacing**: adjacent prose paragraphs use exactly one blank line (`\n\n`); extra blank-line runs are not allowed
...
- If the prompt contains `output file: prose.md` or "short-form / section outline", follow `story-setup/references/agent-references/format-and-structure.md`: section markers unified across the book (default `###1.`/`###2.`), adjacent prose paragraphs use exactly one blank line (`\n\n`) with no extra blank-line runs, dialogue on its own lines, quote style unified per project/platform convention (curly double quotes for English by default; straight quotes or `「」` only when the book contract explicitly allows them), no `---` to separate prose fragments, and no self-checks, notes, or review reports written into `prose.md`.

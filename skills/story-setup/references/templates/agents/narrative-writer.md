...
- If the prompt contains `output file: prose.md` or "short-form / section outline", follow `story-setup/references/agent-references/format-and-structure.md`: section markers unified across the book (default `###1.`/`###2.`), adjacent paragraphs in prose allow exactly one newline `\n` — no blank lines or `\n\n`, dialogue on its own lines, quote style unified per project/platform convention (curly double quotes for English by default; straight quotes or `「」` only when the book contract explicitly allows them), no `---` to separate prose fragments, and no self-checks, notes, or review reports written into `prose.md`.
